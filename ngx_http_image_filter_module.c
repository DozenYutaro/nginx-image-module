...
//	ngx_str_t                    image_name;
    ngx_http_complex_value_t     *namecv;
...
static char *ngx_http_image_filter_image_name(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);
...
static ngx_command_t  ngx_http_image_filter_commands[] = {
    ...
    { ngx_string("image_filter"),
      NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF|NGX_CONF_TAKE13,
      ngx_http_image_filter,
      NGX_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },
      
    { ngx_string("image_filter_image_name"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_LIF_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
	  ngx_http_image_filter_image_name,
	  NGX_HTTP_LOC_CONF_OFFSET,
	  0,
	  NULL },

    { ngx_string("image_filter_image_path"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_LIF_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_slot,
	  NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_image_filter_conf_t, image_path),
	  NULL },
...
ngx_http_image_test(ngx_http_request_t *r, ngx_chain_t *in)
{
	...
	ngx_str_t name;
	ngx_http_complex_value(r, conf->namecv, &name);

    char *image_name = NULL;

	int len = conf->image_path.len + name.len;
	image_name = ngx_pcalloc(r->pool, len+1);
	if(image_name == NULL) {
		return NGX_HTTP_IMAGE_NONE;
	}
	strcpy(image_name, (char*)conf->image_path.data);
	strncat(image_name, (char*)name.data, name.len);
//    FILE *fo;
//    fo = fopen("/home/hdg700/work/amur.net/log", "a");
//    fprintf(fo, "image_name = %s\n", image_name);
//    fclose(fo);
    ...
    if (ctx->image == NULL) {
        ctx->image = ngx_pcalloc(r->pool, ctx->length);
        if (ctx->image == NULL) {
            fclose(f);
            return NGX_ERROR;
        }

        ctx->last = ctx->image + ctx->length;
    }
    int res = fread(ctx->image, 1, ctx->length, f);
	fclose(f);
	
	if(res != (int)ctx->length) {
		return NGX_ERROR;
	}    
...
transparent:
    ...
    if (conf->filter == NGX_HTTP_IMAGE_CROP) {
        ...
        if (ox || oy) {
            ...
            gdImageCopy(dst, src, 0, 0, ox, /*oy*/0, dx - ox, dy - oy);
...
ngx_http_image_filter_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    /* 100 is libjpeg default quality */
    ngx_conf_merge_uint_value(conf->jpeg_quality, prev->jpeg_quality, 100);
    ngx_conf_merge_str_value(conf->image_path, prev->image_path, "default");
    //ngx_conf_merge_str_value(conf->image_name, prev->image_name, "default_name");
...
static char *
ngx_http_image_filter_image_name(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_http_image_filter_conf_t *imcf = conf;
	...
	imcf->namecv = ngx_palloc(cf->pool, sizeof(ngx_http_complex_value_t));
	if (imcf->namecv == NULL) {
		return NGX_CONF_ERROR;
	}
